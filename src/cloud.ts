...
export async function ensureProfile(role: Role, name: string) {
  ...
  const { error } = await supabase.from('profiles').upsert({ id: user.id, name, role }, { onConflict: 'id' }); if (error) throw error; return user
}
export async function getMembership(role: Role, name: string, code?: string) {
  ...
  if (role === 'patient') {
    ...
    const { data: patient, error } = await supabase.from('patients').insert({ name, share_code: shareCode, created_by: user.id }).select('id').single(); if (error) throw error
    const { error: memberError } = await supabase.from('patient_members').insert({ patient_id: patient.id, user_id: user.id, relation: 'patient' }); if (memberError) throw memberError
    return patient.id as string
  }
  if (!code) throw new Error('Informe o código de acompanhamento.')
  const { data: patient, error } = await supabase.from('patients').select('id').eq('share_code', code.trim().toUpperCase()).maybeSingle(); if (error) throw error
  if (!patient) throw new Error('Código de acompanhamento não encontrado.')
  const { error: memberError } = await supabase.from('patient_members').upsert({ patient_id: patient.id, user_id: user.id, relation: 'companion' }, { onConflict: 'patient_id,user_id' }); if (memberError) throw memberError
  return patient.id as string
}
export async function seedPatient(patientId: string) {
  const { count } = await supabase.from('medications').select('id', { count: 'exact', head: true }).eq('patient_id', patientId); if ((count || 0) > 0) return
  const meds = [
  ...
  ]; const { error } = await supabase.from('medications').insert(meds); if (error) throw error
}
export async function loadCloud(patientId: string): Promise<DB> {
  const [{ data: patient, error: pe }, { data: medications, error: me }, { data: events, error: ee }] = await Promise.all([
    supabase.from('patients').select('name,share_code').eq('id', patientId).single(), supabase.from('medications').select('*').eq('patient_id', patientId).order('created_at'), supabase.from('dose_events').select('*').eq('patient_id', patientId).order('scheduled_at')
  ]); if (pe) throw pe; if (me) throw me; if (ee) throw ee
...
export async function insertDose(patientId: string, medId: string, scheduled: string, role: Role) {
  const { data: user } = await supabase.auth.getUser(); const { data: existing } = await supabase.from('dose_events').select('id').eq('medication_id', medId).eq('scheduled_at', scheduled).eq('status', 'confirmed').limit(1); if (existing?.length) throw new Error('Esta dose já foi confirmada em outro dispositivo.')
  const { data, error } = await supabase.from('dose_events').insert({ patient_id: patientId, medication_id: medId, scheduled_at: scheduled, confirmed_at: new Date().toISOString(), confirmed_by: user.user?.id, confirmed_role: role, status: 'confirmed' }).select().single(); if (error) throw error; return data
}
