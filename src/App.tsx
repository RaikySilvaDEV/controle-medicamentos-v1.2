...
function App() {
  ...
  function addMed(name: string, interval: number, form: string, dose: string, note: string) { save({ ...db, meds: [...db.meds, { id: crypto.randomUUID(), name: name.trim(), interval, start: null, active: true, form, dose, note, photo: null }] }); setShowAdd(false); }
  function updateMed(m: Med) { save({ ...db, meds: db.meds.map(x => x.id === m.id ? m : x) }); setDetail(null); }
  function removeMed(m: Med) { if (!confirm(`Excluir ${m.name}?`)) return; save({ ...db, meds: db.meds.filter(x => x.id !== m.id), events: db.events.filter(e => e.medId !== m.id) }); setDetail(null); }
