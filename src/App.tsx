...
function App() {
  ...
  const [role, setRole] = useState<Role>(() => (localStorage.getItem('cm-v12-role') as Role) || 'patient');
  const [patientId, setPatientId] = useState(() => localStorage.getItem('cm-v12-patient-id') || '');
  const [tab, setTab] = useState<Tab>('home'), [now, setNow] = useState(Date.now());
  const [alarm, setAlarm] = useState<{ med: Med; due: Date } | null>(null), [done, setDone] = useState<any>(null);
  ...
  function askNotifications() { if (!('Notification' in window)) return alert('Este navegador não oferece notificações.'); Notification.requestPermission().then(() => setDb(x => ({ ...x, settings: { ...x.settings, notifications: Notification.permission === 'granted' } }))); }
  function testAlarm() { startSound(); window.setTimeout(stopSound, 5000); }
  if (!authChecked) return <div className="grid min-h-screen place-items-center bg-[#f5f7fb] text-slate-500">Carregando…</div>;
  if (!authenticated) return <AuthGate onComplete={(r, name) => { localStorage.setItem('cm-v12-name', name); setRole(r); setAuthenticated(true); }} />;
  if (!onboarded) return <OnboardingGate db={db} onComplete={(r, name, id) => { localStorage.setItem('cm-v12-role', r); localStorage.setItem('cm-v12-patient-id', id); localStorage.setItem('cm-v12-name', name); setRole(r); setPatientId(id); setOnboarded(true); }} />;
  const activePending = alarm || (!done && pending ? { med: pending.m, due: pending.due } : null);
  return <div className="min-h-screen bg-[#f5f7fb] text-[#182033]"><header className="border-b border-slate-100 bg-white"><div className="mx-auto flex max-w-xl items-center justify-between px-5 py-5"><div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-blue-50 text-blue-600"><Pill size={27}/></div><div><div className="text-[10px] font-bold tracking-[.28em] text-slate-400">CONTROLE DE</div><h1 className="text-[27px] font-medium leading-none tracking-tight">Medicamentos</h1><div className="mt-2 flex items-center gap-1.5 text-sm text-slate-500"><UserRound size={14}/> {role === 'patient' ? 'Paciente' : 'Acompanhando'}: {db.patientName}</div></div></div><button onClick={async () => { stopSound(); await supabase.auth.signOut(); }} className="rounded-2xl bg-blue-50 px-5 py-3 font-bold text-blue-600">Sair</button></div></header><main className="mx-auto max-w-xl px-5 pb-28 pt-5">{tab === 'home' && <HomeView db={db} activePending={activePending} next={next} upcoming={upcoming} now={now} onDetail={setDetail}/>} {tab === 'meds' && <MedsView db={db} onAdd={() => setShowAdd(true)} onDetail={setDetail} onPause={pause} onStart={setStartMed}/>} {tab === 'history' && <HistoryView db={db}/>} {tab === 'settings' && <SettingsView db={db} onNotifications={askNotifications} onAlarm={testAlarm}/>}</main><nav className="fixed bottom-0 left-0 right-0 z-30 border-t border-slate-100 bg-white/95 px-2 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 backdrop-blur"><div className="mx-auto grid max-w-xl grid-cols-4 gap-1"><NavButton active={tab === 'home'} icon={<Home size={22}/>} label="Início" onClick={() => setTab('home')}/><NavButton active={tab === 'meds'} icon={<Pill size={22}/>} label="Medicamentos" onClick={() => setTab('meds')}/><NavButton active={tab === 'history'} icon={<History size={22}/>} label="Histórico" onClick={() => setTab('history')}/><NavButton active={tab === 'settings'} icon={<Settings size={22}/>} label="Configurações" onClick={() => setTab('settings')}/></div></nav>{activePending && <AlarmOverlay med={activePending.med} due={activePending.due} now={now} label={role === 'patient' ? 'USADO' : 'ADMINISTRADO'} onConfirm={() => confirmMed(activePending.med)} onSnooze={m => snooze(activePending.med, m)}/>} {done && <ConfirmationOverlay done={done} onClose={() => setDone(null)} onNext={() => { setDone(null); setTab('home'); }}/>} {detail && <DetailSheet med={detail} events={db.events} now={now} onClose={() => setDetail(null)} onConfirm={() => { const due = dueFor(detail, db.events); if (due && due.getTime() <= now) confirmMed(detail); }} onPause={() => pause(detail)} onEdit={() => setEdit(detail)} onDelete={() => removeMed(detail)}/>} {edit && <EditModal med={edit} onClose={() => setEdit(null)} onSave={updateMed}/>} {startMed && <StartTimeModal med={startMed} now={now} onClose={() => setStartMed(null)} onSave={setFirstTime}/>} {showAdd && <AddModal onClose={() => setShowAdd(false)} onSave={addMed}/>}</div>;
}
export default App;
