...
import { Pill, UsersRound } from 'lucide-react';
...
export function AuthGate({ onComplete }: { onComplete: (role: Role, name: string) => void }) {
  const [register, setRegister] = useState(false), [name, setName] = useState(''), [email, setEmail] = useState(''), [pass, setPass] = useState(''), [error, setError] = useState(''), [busy, setBusy] = useState(false), [googleBusy, setGoogleBusy] = useState(false);
  async function submit() {
    ...
    try {
      if (register) {
        const { data, error } = await supabase.auth.signUp({ email: email.trim(), password: pass, options: { data: { name: name.trim() }, emailRedirectTo: getRedirectUrl() } });
        if (error) throw error;
        if (!data.session) { setError('Cadastro criado! Abra o link recebido no e-mail para confirmar sua conta.'); return; }
      } else {
  ...
  async function signInGoogle() {
    ...
    try {
      ...
      if (error) throw error;
    } catch (e: any) { setError(e?.message || 'Não foi possível entrar com Google.'); setGoogleBusy(false); }
  }
  return <div className="min-h-screen bg-[#f5f7fb] px-5 py-10 text-[#182033]"><div className="mx-auto max-w-md rounded-[32px] bg-white p-7 shadow-xl"><div className="mx-auto grid h-20 w-20 place-items-center rounded-[26px] bg-blue-50 text-blue-600"><Pill size={42}/></div><h1 className="mt-6 text-center text-3xl font-bold">Controle de Medicamentos</h1><p className="mt-2 text-center text-slate-500">Organize seus horários e conte com alguém de confiança para acompanhar você.</p><button disabled={googleBusy} onClick={signInGoogle} className="mt-6 flex w-full items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white py-4 font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:opacity-50"><span className="grid h-6 w-6 place-items-center rounded-full bg-white text-lg font-black">G</span>{googleBusy ? 'CONECTANDO...' : 'Continuar com Google'}</button><div className="my-5 flex items-center gap-3 text-xs font-bold text-slate-400"><span className="h-px flex-1 bg-slate-200"/><span>OU</span><span className="h-px flex-1 bg-slate-200"/></div>{register && <input value={name} onChange={e => setName(e.target.value)} placeholder="Nome completo" className="mt-1 w-full rounded-2xl border border-slate-200 p-4"/>}<input value={email} onChange={e => setEmail(e.target.value)} placeholder="E-mail" type="email" className="mt-3 w-full rounded-2xl border border-slate-200 p-4"/><input value={pass} onChange={e => setPass(e.target.value)} placeholder="Senha" type="password" className="mt-3 w-full rounded-2xl border border-slate-200 p-4"/>{error && <div className={`mt-3 rounded-2xl p-3 text-sm font-bold ${error.startsWith('Cadastro criado') ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>{error}</div>}<button disabled={busy} onClick={submit} className="mt-5 w-full rounded-2xl bg-blue-600 py-4 font-black text-white disabled:opacity-50">{busy ? 'AGUARDE...' : register ? 'CRIAR CONTA' : 'ENTRAR'}</button><button onClick={() => { setRegister(!register); setError(''); }} className="mt-3 w-full rounded-2xl border border-blue-200 py-4 font-bold text-blue-600">{register ? 'Já tenho uma conta' : 'Criar uma conta'}</button></div></div>;
}
