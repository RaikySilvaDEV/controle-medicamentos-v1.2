...
/** Precisão baseada no horário previsto salvo em cada dose e no horário real de confirmação. */
export function adherence(db:DB,period:Period){
  const{start,end}=periodRange(period);
  const events=db.events.filter(e=>{const d=parseDate(e.confirmed);return !!d&&d>=start&&d<=end});
  ...
  const precision=confirmed?Math.round(onTime/confirmed*100):0;
  const adherencePct=expected?Math.min(100,Math.round(confirmed/expected*100)):0;
  return{expected,confirmed,late,onTime,missed:overdueUnconfirmed,adherencePct,precision};
}

function medicationStats(db:DB,medId:string,period:Period){
  const{start,end}=periodRange(period);
  const ev=db.events.filter(e=>e.medId===medId).filter(e=>{const d=parseDate(e.confirmed);return !!d&&d>=start&&d<=end});
  const confirmed=ev.length;
  const late=ev.filter(e=>{const s=parseDate(e.scheduled),c=parseDate(e.confirmed);return !!s&&!!c&&c.getTime()-s.getTime()>5*60000}).length;
  const onTime=confirmed-late;
  const med=db.meds.find(m=>m.id===medId);
  const due=med?dueFor(med,db.events):null;
  const missed=due&&due.getTime()<=Date.now()&&due.getTime()>=start.getTime()?1:0;
